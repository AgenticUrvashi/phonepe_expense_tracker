...
function renderKpiCards() {
    ...
    document.getElementById('kpiTxnCount').textContent = `${summary.transaction_count} txns`;
    document.getElementById('kpiAvgSpend').textContent = formatINR(summary.average_transaction);
    document.getElementById('kpiTopCategory').textContent = summary.top_category || 'N/A';

    const healthBadge = document.getElementById('kpiBudgetHealth');
    const kpiStatusCard = document.getElementById('kpiStatusCard');

    if (summary.is_over_budget) {
        const percent = ((summary.total_spent / summary.budget_limit) * 100).toFixed(1);
        healthBadge.innerHTML = `<span style="color:#ef4444;">⚠️ Over by ${formatINR(summary.over_budget_amount)} (${percent}%)</span>`;
        if (kpiStatusCard) kpiStatusCard.className = 'glass-panel kpi-card danger';
    } else {
        const percent = ((summary.total_spent / summary.budget_limit) * 100).toFixed(1);
        healthBadge.innerHTML = `<span style="color:#10b981;">✅ Safe (${percent}% used, ${formatINR(summary.remaining_budget)} left)</span>`;
        if (kpiStatusCard) kpiStatusCard.className = 'glass-panel kpi-card success';
    }
}
